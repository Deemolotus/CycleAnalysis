"""
═══════════════════════════════════════════════════════════════════
功能：Cycle + TD9 + SQZ Momentum + SuperTrend + Chaikin Money Flow (CMF)
═══════════════════════════════════════════════════════════════════
"""
...
PHASING_SCALE       = 209
DEFAULT_HTML        = str(Path("tmp") / "quant_sniper_v8.html")
YF_CACHE_DIR        = Path("tmp") / "yf_cache"
PASS_COLOR          = "#59c36a"
FAIL_COLOR          = "#e36a6a"
...
#  GUI (FULLY RESTORED)
# ══════════════════════════════════════════════════════════════════════════════
BG = "#12141c"; PANEL = "#1c1f2e"; INPUT_BG = "#252840"; BORDER = "#323659"; TEXT_FG = "#dde1f0"; MUTED = "#676c8a"
ACCENT = "#5c7cfa"; SUCCESS = "#51cf66"; ERR = "#ff6b6b"; WARN = "#f3a53a"; BTN_RUN = "#4361ee"; BTN_HOV = "#3451d1"

class CycleChartApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Quant Sniper Ultimate v8.0 ── Dynamic Frequency")
        self.configure(bg=BG); self.geometry("460x950"); self.minsize(420, 700)
        self._running = False; self._setup_style(); self._build_ui()

    def _setup_style(self):
        s = ttk.Style(self); s.theme_use("clam")
        for w in (".", "TFrame", "TLabel", "TCheckbutton"): s.configure(w, background=BG, foreground=TEXT_FG, font=("Segoe UI", 10))
        s.configure("Panel.TFrame", background=PANEL); s.configure("P.TLabel", background=PANEL, foreground=TEXT_FG, font=("Segoe UI", 10))
        s.configure("H.TLabel", background=PANEL, foreground=ACCENT, font=("Segoe UI", 15, "bold")); s.configure("Sec.TLabel", background=PANEL, foreground=ACCENT, font=("Segoe UI", 10, "bold"))
        s.configure("Muted.TLabel", background=PANEL, foreground=MUTED, font=("Segoe UI", 8))
        s.configure("TCheckbutton", background=PANEL, foreground=TEXT_FG, indicatorcolor=INPUT_BG, font=("Segoe UI", 9)); s.map("TCheckbutton", indicatorcolor=[("selected", ACCENT)])
        s.configure("TEntry", fieldbackground=INPUT_BG, foreground=TEXT_FG, insertcolor=TEXT_FG, bordercolor=BORDER, lightcolor=BORDER, darkcolor=BORDER, relief="flat")
        s.map("TEntry", bordercolor=[("focus", ACCENT)])
        s.configure("Run.TButton", background=BTN_RUN, foreground="white", font=("Segoe UI", 11, "bold"), relief="flat", borderwidth=0, padding=(0, 12))
        s.map("Run.TButton", background=[("active", BTN_HOV), ("disabled", "#2a2e45")], foreground=[("disabled", MUTED)])
        s.configure("Sm.TButton", background=INPUT_BG, foreground=TEXT_FG, font=("Segoe UI", 9), relief="flat", borderwidth=0, padding=(4, 3))
        s.configure("TProgressbar", troughcolor=INPUT_BG, background=ACCENT, thickness=4)

    def _build_ui(self):
        self.columnconfigure(0, weight=1); self.rowconfigure(0, weight=1)
        outer = ttk.Frame(self, style="Panel.TFrame"); outer.grid(row=0, column=0, sticky="nsew", padx=8, pady=8)
        outer.columnconfigure(0, weight=1); outer.rowconfigure(0, weight=1)

        # Scrollable Canvas
        canvas = tk.Canvas(outer, bg=PANEL, highlightthickness=0); vbar = ttk.Scrollbar(outer, orient="vertical", command=canvas.yview)
        canvas.configure(yscrollcommand=vbar.set)
        self._sf = ttk.Frame(canvas, style="Panel.TFrame"); self._sf.columnconfigure(0, weight=1)
        ...
        self._build_sidebar(self._sf)

    def _lbl(self, p, text, style="P.TLabel"): return ttk.Label(p, text=text, style=style)
    def _entry_row(self, p, row, label, var, hint=""):
        self._lbl(p, label).grid(row=row, column=0, sticky="w", padx=16, pady=(7, 1))
        ttk.Entry(p, textvariable=var).grid(row=row+1, column=0, sticky="ew", padx=16, pady=(0, 1))
        if hint:
            ttk.Label(p, text=hint, style="Muted.TLabel").grid(row=row+2, column=0, sticky="w", padx=16, pady=(0, 2))
            return row+3
        return row+2
    def _sep(self, p, row): ttk.Separator(p, orient="horizontal").grid(row=row, column=0, sticky="ew", padx=12, pady=6); return row+1

    def _build_sidebar(self, p):
        r = 0
        ttk.Label(p, text="Ultimate Quant Sniper", style="H.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(18, 0)); r += 1
        ttk.Label(p, text="v8.0 — Dynamic Frequency Extraction (DFE)", style="Muted.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(0, 8)); r += 1
        r = self._sep(p, r)

        # --- 1. Cycle & Base Params ---
        ttk.Label(p, text="CYCLE PARAMETERS", style="Sec.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(2, 4)); r += 1
        self._sym = tk.StringVar(value=DEFAULT_SYMBOL); self._end_date = tk.StringVar(value=date.today().isoformat())
        self._start_date = tk.StringVar(value=""); self._lookback = tk.StringVar(value="")
        self._future_days = tk.StringVar(value=str(DEFAULT_FUTURE_DAYS))

        r = self._entry_row(p, r, "Ticker Symbol", self._sym)
        r = self._entry_row(p, r, "End Date (YYYY-MM-DD)", self._end_date)
        r = self._entry_row(p, r, "Start Date (optional)", self._start_date, "Leave blank → auto lookback (678–900)")
        r = self._entry_row(p, r, "Lookback Bars (optional)", self._lookback)
        r = self._entry_row(p, r, "Future Projection (days)", self._future_days)

        self._auto_dfe = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            p, text="Auto-detect cycle periods (DFE)", variable=self._auto_dfe,
            command=self._toggle_dfe_periods,
        ).grid(row=r, column=0, sticky="w", padx=18, pady=(4, 2)); r += 1

        self._lbl(p, "Cycle Periods (Days)").grid(row=r, column=0, sticky="w", padx=16, pady=(7, 2)); r += 1
        pf = ttk.Frame(p, style="Panel.TFrame"); pf.grid(row=r, column=0, sticky="ew", padx=16, pady=(0, 4)); r += 1
        self._p1, self._p2, self._p3 = (tk.StringVar(value=str(v)) for v in DEFAULT_PERIODS)
        self._period_entries = []
        for col, sv in enumerate([self._p1, self._p2, self._p3]):
            ent = ttk.Entry(pf, textvariable=sv, width=6)
            ent.grid(row=0, column=col, padx=(0, 6))
            self._period_entries.append(ent)
        self._toggle_dfe_periods()
        r = self._sep(p, r)

        # --- 2. SuperTrend Params ---
        ttk.Label(p, text="SUPERTREND", style="Sec.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(2, 6)); r += 1
        self._st_show = tk.BooleanVar(value=True)
        ttk.Checkbutton(p, text="启用 SuperTrend 主图红绿带", variable=self._st_show).grid(row=r, column=0, sticky="w", padx=18, pady=1); r += 1
        stf = ttk.Frame(p, style="Panel.TFrame"); stf.grid(row=r, column=0, sticky="ew", padx=16, pady=(6, 2)); r += 1
        self._st_len = tk.StringVar(value="10"); self._st_mult = tk.StringVar(value="3.0")
        ttk.Label(stf, text="ATR Period:").grid(row=0, column=0); ttk.Entry(stf, textvariable=self._st_len, width=5).grid(row=0, column=1, padx=(4, 12))
        ttk.Label(stf, text="Multiplier:").grid(row=0, column=2); ttk.Entry(stf, textvariable=self._st_mult, width=5).grid(row=0, column=3, padx=(4, 0))
        r = self._sep(p, r)

        # --- 3. TD Sequential Params ---
        ttk.Label(p, text="TD SEQUENTIAL (DeMark)", style="Sec.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(2, 6)); r += 1
        self._td_show, self._td_setup, self._td_cd, self._td_nums = tk.BooleanVar(value=True), tk.BooleanVar(value=True), tk.BooleanVar(value=True), tk.BooleanVar(value=False)
        for var, text in [(self._td_show, "启用 TD 信号叠加"), (self._td_setup, "显示 TD9 Setup 标记"), (self._td_cd, "显示 TD13/15 Countdown"), (self._td_nums, "显示逐根计数 (缩放后看)")]:
            ttk.Checkbutton(p, text=text, variable=var).grid(row=r, column=0, sticky="w", padx=18, pady=1); r += 1
        cf = ttk.Frame(p, style="Panel.TFrame"); cf.grid(row=r, column=0, sticky="ew", padx=16, pady=(6, 2)); r += 1
        self._cd_tgt, self._cd_ext = tk.StringVar(value="13"), tk.StringVar(value="15")
        ttk.Label(cf, text="Countdown:").grid(row=0, column=0); ttk.Entry(cf, textvariable=self._cd_tgt, width=4).grid(row=0, column=1, padx=(6, 12))
        ttk.Label(cf, text="Extended:").grid(row=0, column=2); ttk.Entry(cf, textvariable=self._cd_ext, width=4).grid(row=0, column=3, padx=(6, 0))
        r = self._sep(p, r)

        # --- 4. Squeeze Momentum Params ---
        ttk.Label(p, text="SQUEEZE MOMENTUM [LazyBear]", style="Sec.TLabel").grid(row=r, column=0, sticky="w", padx=16, pady=(2, 6)); r += 1
        self._sqz_show = tk.BooleanVar(value=True)
        ttk.Checkbutton(p, text="启用 SQZ 动量副图", variable=self._sqz_show).grid(row=r, column=0, sticky="w", padx=18, pady=(0, 6)); r += 1
        sqzf = ttk.Frame(p, style="Panel.TFrame"); sqzf.grid(row=r, column=0, sticky="ew", padx=16, pady=(0, 2)); r += 1
        self._sqz_bb_len, self._sqz_bb_mult, self._sqz_kc_len, self._sqz_kc_mult = tk.StringVar(value="20"), tk.StringVar(value="2.0"), tk.StringVar(value="20"), tk.StringVar(value="1.5")
        ttk.Label(sqzf, text="BB Length:").grid(row=0, column=0, sticky="w"); ttk.Entry(sqzf, textvariable=self._sqz_bb_len, width=5).grid(row=0, column=1, padx=(4, 12))
        ttk.Label(sqzf, text="BB Mult:").grid(row=0, column=2, sticky="w"); ttk.Entry(sqzf, textvariable=self._sqz_bb_mult, width=5).grid(row=0, column=3, padx=(4, 0))
        ttk.Label(sqzf, text="KC Length:").grid(row=1, column=0, sticky="w", pady=(4, 0)); ttk.Entry(sqzf, textvariable=self._sqz_kc_len, width=5).grid(row=1, column=1, padx=(4, 12), pady=(4, 0))
        ttk.Label(sqzf, text="KC Mult:").grid(row=1, column=2, sticky="w", pady=(4, 0)); ttk.Entry(sqzf, textvariable=self._sqz_kc_mult, width=5).grid(row=1, column=3, padx=(4, 0), pady=(4, 0))
        r = self._sep(p, r)

        # --- 5. Output Path & Run Controls ---
        self._lbl(p, "Save HTML to").grid(row=r, column=0, sticky="w", padx=16, pady=(4, 2)); r += 1
        of = ttk.Frame(p, style="Panel.TFrame"); of.grid(row=r, column=0, sticky="ew", padx=16, pady=(0, 4)); r += 1; of.columnconfigure(0, weight=1)
        self._output = tk.StringVar(value=DEFAULT_HTML)
        ttk.Entry(of, textvariable=self._output).grid(row=0, column=0, sticky="ew")
        ttk.Button(of, text="…", style="Sm.TButton", width=2, command=self._browse_output).grid(row=0, column=1, padx=(4, 0))

        self._progress = ttk.Progressbar(p, mode="indeterminate"); self._progress.grid(row=r, column=0, sticky="ew", padx=16, pady=(6, 4)); r += 1
        self._run_btn = ttk.Button(p, text="▶ Run & Analyze", style="Run.TButton", command=self._start_run); self._run_btn.grid(row=r, column=0, sticky="ew", padx=16, pady=(0, 4)); r += 1

        bf = ttk.Frame(p, style="Panel.TFrame"); bf.grid(row=r, column=0, sticky="ew", padx=16, pady=(0, 6)); r += 1; bf.columnconfigure(0, weight=1)
        self._reopen_btn = ttk.Button(bf, text="Re-open in Browser", style="Sm.TButton", command=self._reopen, state="disabled"); self._reopen_btn.grid(row=0, column=0, sticky="ew")

        # --- 6. Console Log ---
        r = self._sep(p, r)
        ttk.Label(p, text="Engine Output", foreground=MUTED, font=("Segoe UI", 9, "bold")).grid(row=r, column=0, sticky="w", padx=16); r += 1
        logf = ttk.Frame(p); logf.grid(row=r, column=0, sticky="ew", padx=8, pady=(2, 16)); r += 1; logf.columnconfigure(0, weight=1)
        self._log = tk.Text(logf, height=12, bg="#0c0d14", fg="#8892b0", font=("Consolas", 9), bd=0, relief="flat", wrap="word", state="disabled")
        self._log.grid(row=0, column=0, sticky="ew")
        sb = ttk.Scrollbar(logf, orient="vertical", command=self._log.yview); sb.grid(row=0, column=1, sticky="ns"); self._log["yscrollcommand"] = sb.set

    def _toggle_dfe_periods(self):
        state = "disabled" if self._auto_dfe.get() else "normal"
        for ent in getattr(self, "_period_entries", []):
            ent.configure(state=state)

    def _browse_output(self):
        p = filedialog.asksaveasfilename(defaultextension=".html", filetypes=[("HTML", "*.html"), ("All", "*.*")], initialfile="quant_sniper.html")
    ...
    def _reopen(self):
        if hasattr(self, '_last_html') and self._last_html: webbrowser.open(f"file:///{Path(self._last_html).resolve()}")

    def _start_run(self):
        if self._running: return
        self._running = True; self._run_btn.state(["disabled"]); self._progress.start(10)
        self._log.configure(state="normal"); self._log.delete("1.0", "end"); self._log.configure(state="disabled")
        threading.Thread(target=self._run_analysis, daemon=True).start()

    def _run_analysis(self):
        try:
            # Gather all variables from the restored UI
            symbol = self._sym.get().strip() or DEFAULT_SYMBOL
            end_date = self._end_date.get().strip() or date.today().isoformat()
            start_date = self._start_date.get().strip()
            lb_str = self._lookback.get().strip()
            future_days = int(self._future_days.get())
            auto_dfe = self._auto_dfe.get()
            periods_manual = [float(sv.get().strip()) for sv in [self._p1, self._p2, self._p3] if sv.get().strip()] or None

            show_st = self._st_show.get()
            st_len, st_mult = int(self._st_len.get()), float(self._st_mult.get())

            show_td, show_setup, show_cd, show_nums = self._td_show.get(), self._td_setup.get(), self._td_cd.get(), self._td_nums.get()
            cd_tgt, cd_ext = int(self._cd_tgt.get()), int(self._cd_ext.get())

            show_sqz = self._sqz_show.get()
            sqz_bb_len, sqz_bb_mult = int(self._sqz_bb_len.get()), float(self._sqz_bb_mult.get())
            sqz_kc_len, sqz_kc_mult = int(self._sqz_kc_len.get()), float(self._sqz_kc_mult.get())

            out_path = Path(self._output.get().strip() or DEFAULT_HTML)

            self._log_write(f"Fetching Data for {symbol}...")
            ...
            if start_date:
                ...
                auto_lookback = True
                start_date = (pd.Timestamp(end_date) - pd.offsets.BDay(MAX_LOOKBACK_BARS)).date().isoformat()
                self._log_write("Auto lookback: fetching max window for DFE + sweep…")

            close, high, low, volume = download_ohlcv(symbol, start_date, end_date)
            self._log_write(f"  Downloaded {len(close)} bars")

            if auto_lookback:
                self._log_write("Auto-selecting lookback (period-driven min + stability sweep)…")
            ...
            cfg = resolve_cycle_config(
                ...
                log_cb=lambda m: self._log_write(m),
            )
            ...
            if auto_dfe and len(periods) >= 3:
                p0, p1, p2 = periods[0], periods[1], periods[2]
                def _upd_period_fields():
                    self._p1.set(f"{p0:.1f}"); self._p2.set(f"{p1:.1f}"); self._p3.set(f"{p2:.1f}")
                self.after(0, _upd_period_fields)

            m = metrics_for_window(close, periods, future_days)
            overlay, cs = m["overlay"], m["cycle_stats"]
            self._log_write(
                f"Fit R²={m['combined_r2']:.4f}  ranked={[int(p) for p in m['ranked_periods']]}"
            )
            self._log_write(m["timing_report"])

            trend, st_line = calculate_supertrend(high, low, close, st_len, st_mult)
            _, _, sigs = td_sequential(close, high, low, cd_tgt, cd_ext)
            sqz_val, _, _, _, _, _ = squeeze_momentum(close, high, low, sqz_bb_len, sqz_bb_mult, sqz_kc_len, sqz_kc_mult)
            cmf_val = chaikin_money_flow(close, high, low, volume)

            # Analyze and log
            trade_advice = analyze_trade_state(close, low, high, trend, st_line, sigs, sqz_val, cmf_val)
            self._log_write(trade_advice)

            # Build and render chart using all UI settings
            fig = build_plotly_chart(
                close, overlay, cs, symbol, periods, high, low,
                show_td, show_setup, show_cd, show_nums, cd_tgt, cd_ext,
                show_sqz, sqz_bb_len, sqz_bb_mult, sqz_kc_len, sqz_kc_mult,
                show_st, st_len, st_mult, cmf_val,
                projection_extrema=m.get("projection_extrema"),
            )

            out_path.parent.mkdir(parents=True, exist_ok=True)
            fig.write_html(str(out_path), include_plotlyjs="cdn", config={"scrollZoom": True, "displaylogo": False})
            self._last_html = str(out_path)
            webbrowser.open(f"file:///{out_path.resolve()}")
            self.after(0, lambda: self._reopen_btn.state(["!disabled"]))

        except Exception as e:
            import traceback
            self._log_write(f"Error:\n{traceback.format_exc()}")
        finally:
            self._running = False; self.after(0, self._progress.stop); self.after(0, lambda: self._run_btn.state(["!disabled"]))
